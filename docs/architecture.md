...
**压缩期与保留期是两个独立阈值**：压缩默认从 2 天开始，删除则等到保留期
（先 30 天、后 7 天）；压缩期短于保留期时，digest 已经写好但原文仍在，
不会提前删除。读窗口现在是 72 小时 / 最多 400 条 / 最多 6 万字符，窗口与
压缩期（2 天）有意重叠：窗口内的原文保证逐字上下文，窗口外（含被删除的区间）
由画像与 digest 承担，两者共同覆盖全部历史。
