...
| 分条发送 | 按人类消息基线（单条中位数 8 字、一轮中位数 10 字）设定长度护栏：通常 1 条、最多 3 条短气泡（单条尽量 12 字、总字数尽量 20 字）；超过目标长度的气泡按标点切短，溢出时用逗号连接后均衡合并；超过 30 字硬预算的内容从尾部裁剪并在日志标注；`msg_seq` 递增，条间隔按上一条长度自适应 |
...
| `BUBBLE_TARGET_MAX_CHARS` | 12 | 单条气泡目标长度（人类单条中位数 8 字） |
| `MAX_REPLY_TOTAL_CHARS` | 30 | 单次回复总字数硬上限，超出从尾部裁剪并记 trimmed-total |
...
标点与语气词（句号、~ 等）不做硬性限制，由模型按语境自然输出；只清理气泡切分产生的悬空分隔标点（，、；：）。人类基线样本目前仅两人，上述长度护栏也保持保守，不当作风格规则。
...
Reply parse warning: merged-overflow      超过条数上限，已均衡合并
Reply parse warning: trimmed-total         超过总字数硬预算，已从尾部裁剪
