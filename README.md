...
| 分条发送 | 模型在 JSON 输出模式下返回消息数组（1～3 条完整气泡）；气泡内的换行/空行会自动拆成独立气泡，超限合并进最后一条；`msg_seq` 递增，条间隔按上一条长度自适应 |
...
| 决策协议 | 模型输出 `{"silent":true}` 表示不回复、`{"messages":[...]}` 表示回复；DeepSeek 请求启用 `response_format: json_object`；旧 `NO_REPLY` 与纯文本仍兼容兜底 |
