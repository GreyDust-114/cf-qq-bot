...
| 分条发送 | 按人类消息基线（单条中位数 8 字、一轮中位数 10 字）设定：通常 1 条、最多 3 条短气泡（单条尽量 12 字、总字数尽量 20 字，句尾不加句号）；超过目标长度的气泡按标点切短，溢出时用逗号连接后均衡合并；超过 30 字硬预算的内容从尾部裁剪并在日志标注；`msg_seq` 递增，条间隔按上一条长度自适应 |
